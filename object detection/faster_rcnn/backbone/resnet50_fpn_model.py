...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        identity = x
        if self.downsample is not None:# 如果为None 为实线残差结构，不为None 则对应虚线残差结构
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        ...
        self.layer1 = self._make_layer(block, 64, blocks_num[0]) #默认stride = 1
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        if stride != 1 or self.in_channel != channel * block.expansion:
            downsample = nn.Sequential(
                ...
                norm_layer(channel * block.expansion)) #虚线分支结构
        ...
        for _ in range(1, block_num): #由于从第2层开始均为实线残差结构
