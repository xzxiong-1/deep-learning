...
class Backbone(nn.Module):
    def __init__(self, pretrain_path=None):
        ...
        net = resnet50()
        self.out_channels = [1024, 512, 512, 256, 256, 256]  #对应各个预测特征层的backbone

        if pretrain_path is not None: #是否有预训练模型
            net.load_state_dict(torch.load(pretrain_path))

        self.feature_extractor = nn.Sequential(*list(net.children())[:7]) # 由nn方法生成的  提取子模块索引为0到6的位置conv1到layer3
                                                                        # *表示非关键字参数
        conv4_block1 = self.feature_extractor[-1][0] #对应conv4_x的第一个子模块block1

        # 修改conv4_block1的步距，从2->1
        conv4_block1.conv1.stride = (1, 1) #ResNet可忽略
        conv4_block1.conv2.stride = (1, 1) 
        conv4_block1.downsample[0].stride = (1, 1) #残差分支
...
class SSD300(nn.Module):
    def __init__(self, backbone=None, num_classes=21):
        ...
        if backbone is None:  #判断backbone 是否为空
        ...
        # out_channels = [1024, 512, 512, 256, 256, 256] for resnet50  6 个预测特征层的channels
        self._build_additional_features(self.feature_extractor.out_channels) #backbone 后面的一系列结构
        # 构建预测器
        self.num_defaults = [4, 6, 6, 6, 4, 4] #每个cell上的default box
        location_extractors = []
        ...
        for nd, oc in zip(self.num_defaults, self.feature_extractor.out_channels):
            ...
            location_extractors.append(nn.Conv2d(oc, nd * 4, kernel_size=3, padding=1)) #nd * 4 表示每个default box的预测变量个数
    ...
    def _build_additional_features(self, input_size):
        ...
        middle_channels = [256, 256, 128, 128, 128] #后5个的第一个残差结构
        for i, (input_ch, output_ch, middle_ch) in enumerate(zip(input_size[:-1], input_size[1:], middle_channels)):
            padding, stride = (1, 2) if i < 3 else (0, 1) #后5个层结构的前三个 以及后两个
            layer = nn.Sequential(
...
class Loss(nn.Module):
    ...
    def __init__(self, dboxes):
        ...
        self.scale_xy = 1.0 / dboxes.scale_xy  # 10  scale_xy 0.1
        self.scale_wh = 1.0 / dboxes.scale_wh  # 5   scale_wh 0.2

        # 定位损失
        self.location_loss = nn.SmoothL1Loss(reduction='none')
        # num_anchors 8732
        # [num_anchors, 4] -> [4, num_anchors] -> [1, 4, num_anchors] #unseueeze 用于增加维度
    ...
    def forward(self, ploc, plabel, gloc, glabel):
        ...
        mask = torch.gt(glabel, 0)  # (gt: >)   #得到所有匹配到gtbox的default box
        # mask1 = torch.nonzero(glabel)
        # 计算一个batch中的每张图片的正样本个数 Tensor: [N]
        pos_num = mask.sum(dim=1)  #example [8, 17, 18, 15, 10, 11, 10, 13]
        ...
        # 计算定位损失(只有正样本)
        loc_loss = self.location_loss(ploc, vec_gd).sum(dim=1)  # 在xywh维度上求和 Tensor: [N, 8732]
        loc_loss = (mask.float() * loc_loss).sum(dim=1)  # Tenosr: [N] 每张图片的定位损失

        # hard negative mining Tenosr: [N, 8732] 分类损失
        con = self.confidence_loss(plabel, glabel)

        # positive mask will never selected
        # 获取负样本
        con_neg = con.clone()
        con_neg[mask] = 0.0 #正样本损失置位0
        # 按照confidence_loss降序排列 con_idx(Tensor: [N, 8732])
        _, con_idx = con_neg.sort(dim=1, descending=True) #idx  索引 降序
        _, con_rank = con_idx.sort(dim=1)  # 这个步骤比较巧妙 #升序

        # number of negative three times positive
        # 用于损失计算的负样本数是正样本的3倍（在原论文Hard negative mining部分），
        # 但不能超过总样本数8732= mask.size(1)
        neg_num = torch.clamp(3 * pos_num, max=mask.size(1)).unsqueeze(-1)
        neg_mask = torch.lt(con_rank, neg_num)  # (lt: <) Tensor [N, 8732]

        # confidence最终loss使用选取的正样本loss+选取的负样本loss  分类损失x(正样本mask+负样本mask)=最终分类损失
